...
- Model capabilities live in `server/video.ts`: Grok and MiniMax H3 Max accept only a start frame; MiniMax H3 and Seedance accept both together. H3 Max's OpenRouter route rejects paired frames, while MiniMax rejects `last_frame` without `first_frame` (checked 2026-09-14), so end frames are unusable despite the discovery metadata. Keep `supportsEndImage` separate from `maxKeyframeImages`; listing supported types does not guarantee valid combinations. Share validation with the client and reject unsupported combinations before a provider call.
